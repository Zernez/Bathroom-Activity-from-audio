...
class PredictorRsn152:

    def __init__(self):  
        self.activities= ['toilet_flush', 'showering', 'vacuum_cleaner', 'brushing_teeth', 'washing_machine'] 
        self.img_x= 128
        self.img_y= 431        
        self.resnet_model_152 = resnet152()
        self.learning_rate = 2e-4
        self.optimizer = optim.Adam(self.resnet_model_152.parameters(), lr= self.learning_rate)
        self.resnet_model_152 = self.load_model()

    def load_model(self):
        num_classes = len (self.activities)
        num_ftrs= 2048
        self.resnet_model_152.fc= nn.Linear(num_ftrs, num_classes)
        self.resnet_model_152, optimizer= self.load_ckp('./models/resnet152.pth', self.resnet_model_152, self.optimizer)
        self.resnet_model_152.eval()
        return self.resnet_model_152
    ...
    def predict_model(self, last):
        ...
        pr= self.resnet_model_152.forward(spec_t)
