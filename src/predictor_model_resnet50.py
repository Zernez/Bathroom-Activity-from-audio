...
class PredictorRsn50:

    def __init__(self):  
        self.activities= ['toilet_flush', 'showering', 'vacuum_cleaner', 'brushing_teeth', 'washing_machine'] 
        self.img_x= 128
        self.img_y= 431        
        self.resnet_model_50 = resnet50()
        self.learning_rate = 2e-4
        self.optimizer = optim.Adam(self.resnet_model_50.parameters(), lr= self.learning_rate)
        self.resnet_model_50 = self.load_model()

    def load_model(self):
        num_classes = len (self.activities)
        num_ftrs= 2048
        self.resnet_model_50.fc= nn.Linear(num_ftrs, num_classes)
        self.resnet_model_50, optimizer= self.load_ckp('./models/resnet50.pth', self.resnet_model_50, self.optimizer)
        self.resnet_model_50.eval()
        return self.resnet_model_50
    ...
    def predict_model(self, last):
        ...
        pr= self.resnet_model_50.forward(spec_t)
